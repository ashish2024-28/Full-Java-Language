...
// ********** (self) DomainAdmin operations **********
    ...
    public DomainAdmin LoginDomainAdmin(String domain, DomainAdmin loginDAdmin){
        DomainAdmin dAdminG = dAdminRepo.findByDomainAndEmail(domain,loginDAdmin.getEmail());
        DomainAdmin dAdminP = dAdminRepo.findByEmailAndPassword(loginDAdmin.getEmail(),loginDAdmin.getPassword());
//        if ((dAdminG.getDomain() == dAdminP.getDomain() && dAdminG.getGmail() == dAdminP.getGmail())&& dAdminG.getPassword() == dAdminP.getPassword()) {
        if ((Objects.equals(dAdminG.getDomain(), dAdminP.getDomain()) && Objects.equals(dAdminG.getEmail(), dAdminP.getEmail()))&& Objects.equals(dAdminG.getPassword(), dAdminP.getPassword())) {
            dAdminG.setLastLoginDateTime(LocalDateTime.now());
            return dAdminG;   
        } 
        else {
            return null;
